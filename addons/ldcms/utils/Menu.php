...
class Menu
{
    ...
    public static function create($menu = [], $parent = 0)
    {
        ...
        if ($matches && isset($matches[1])) {
            \app\common\library\Menu::refresh($matches[1], $menu);
        }
    }
     ...
     * 更新某个菜单及子菜单
     * @param string $name             插件名称
     * @param string $updateMenuName   更新的菜单名称
     * @param array $newMenu          新菜单
     * @return bool
     * @throws Exception
     ...
     */
    public static function update($name,$updateMenuName,$newMenu)
    {
        $ids = self::getAuthRuleIdsByName($updateMenuName);
        $old = AuthRule::where('id', 'in', $ids)->select();
        if(collection($old)->isEmpty()){
            return true;
        }
        $old = collection($old)->toArray();
        $oldfirst=$old[0];
        ...
        Db::startTrans();
        try {
            self::menuUpdate($newMenu, $old,$oldfirst['pid']);
            $ids = [];
            foreach ($old as $index => $item) {
                if (!isset($item['keep'])) {
                    $ids[] = $item['id'];
